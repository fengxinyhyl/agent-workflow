...
| `strategy.research.agent_workflow.adapters/` | Partial reuse | Claude CLI / Codex CLI adapter **行为已移植到 root**（Phase C/D 完成，2026-06-11）；legacy adapter 代码仍可 import，但 root adapter 已包含等价 observability + safety 能力 |
...
| 指标 | 2026-06-09 baseline | 2026-06-11 (Phase C/D/E 后) |
|------|---------------------|------------------------------|
| Exit code | 0 | 0 |
| Collected | 211 | 247 |
| Passed | 211 | 247 |
...
**评估 (2026-06-11):** Root 测试套件全部通过。新增 36 个测试覆盖 Phase C0/C/D（streaming / ClaudeCLI / CodexCLI / sandbox 安全拦截）。

### 7.3 Adapter 字段级 Parity 对照表 (2026-06-11, Phase C/D 完成后)

| 字段 | legacy WorkerResult | root TaskResult | 对齐状态 |
|------|---------------------|-----------------|----------|
| worker/agent | `worker` | `agent` | ✅ equivalent |
| status | `status` | `status` | ✅ equivalent |
| exit_code | `exit_code` | `execution.exit_code` | ✅ equivalent |
| session_id | `session_id` | `session_id` | ✅ Phase C/D 已填充 (Claude: --session-id uuid; Codex: thread.started.thread_id) |
| pid | `pid` | `execution.pid` | ✅ Phase C/D 已填充 |
| log_path | `log_path` | `log_path` | ✅ Phase C/D 已填充 (stream JSONL 逐行落盘) |
| packet_path | `packet_path` | `packet_path` | ✅ Phase C/D 已填充 (packets/<state>_<agent>_last_message.md) |
| token_usage | `token_usage` | `token_usage` | ✅ Phase C/D 已填充 (Claude: parse_claude_usage; Codex: turn.completed.usage) |
| output_path | `output_path` | `artifacts[].artifact_path` | ✅ equivalent |
| started_at | `started_at` | `execution.started_at` | ✅ equivalent |
| finished_at | `finished_at` | `execution.finished_at` | ✅ equivalent |
| duration | 由 started/finished 计算 | `execution.duration_seconds` | ✅ equivalent |
| error | `error` | `issues[]` (severity=blocking) | ✅ equivalent |

**消费路径确认：**
- root `agent-workflow` 中 `log_path` 的消费者是 `workflow run` 的 `tail`/`log` 命令
- 当前 Phase C/D 填充 `log_path` 后，下游可直接通过 `TaskResult.log_path` 定位，无需硬编码路径

**Schema 策略决策：**
- `token_usage` 在 schema 中保持 `type: object`，`additionalProperties` 隐式 true
- 允许 Claude 使用 `cache_read_input_tokens`、Codex 使用 `cached_input_tokens` / `reasoning_output_tokens` 而不触发 schema validation 错误
