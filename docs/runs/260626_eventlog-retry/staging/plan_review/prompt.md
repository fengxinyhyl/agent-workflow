...
输入: plan_doc:latest, plan_doc:all, plan_refinement_doc:latest, project_context

期望输出: plan_review_doc
...
- plan_review_doc: artifacts/plan_review_doc-v1.md
...
状态历史: planning → plan_review → plan_refinement → plan_review
...
明确列出 blocking issues、major risks、missing evidence、
suggested revisions 和 do-not-change constraints。
...

{
  ...
  "properties": {
    ...
    "decision": {
      "type": "string",
      "description": "语义决策（允许值: approve, revise, reject, fail, blocked）",
      "enum": [
        "approve",
        "revise",
        "reject",
...
- plan_review_doc: F:\code\agent-workflow\docs\runs\260626_eventlog-retry\staging\plan_review\plan_review_doc.md
- task_result: F:\code\agent-workflow\docs\runs\260626_eventlog-retry\staging\plan_review\task_result.json
...
⚠️ **本任务的 `decision` 字段必须从以下值中选择一个**：approve, blocked, fail, reject, revise。不要使用列表之外的值（例如不要用 done 代替 approve）。
