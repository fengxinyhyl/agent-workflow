...
- `done`: 计划完整、可审核。
- `blocked`: 缺少关键上下文，无法制定可靠计划。
- `fail`: 目标明显不可行或和安全边界冲突。
...
- plan_doc: F:\code\agent-workflow\docs\runs\260626_eventlog-retry\staging\planning\plan_doc.md
- task_result: F:\code\agent-workflow\docs\runs\260626_eventlog-retry\staging\planning\task_result.json
