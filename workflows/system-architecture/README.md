...
3. **改着改着就跑偏，前面定的结论被后面悄悄推翻。** 评估阶段明明判"这条已定"，冻结文档里却写成"待定"；一次约束放宽没人签字就生效了。
4. **架构里画的表，落库时才发现撞了既有表名或迁移编号。** 方案层的数据模型和真实数据库基线脱节，冲突拖到开发期才爆。

这个工作流分别用：约束驱动 + 逐条对齐、异源独立评估、两道确定性脚本门、数据模型确定性投影 —— 来正面应对这四点。

---

## 工作流结构（约束驱动 + 评估闭环 + 确定性投影）

八个建模节点，外加两道确定性脚本门。前段是"发散—收敛"的评估闭环，后段是"冻结—投影—校验"的确定性链。
...
flowchart TD
    ...
    subgraph EVAL["评估闭环（发散到收敛）"]
        ...
        EG{{"evaluation_gate<br/>评分矩阵，违反项，权衡冲突"}}
...
> 图例：🟦 模型节点 ｜ 🟧 评估门（六边形，LLM 打分裁决）｜ 🟩 确定性脚本门（双线框，机器兜底、与评估门正交）｜ 🟪 产物（斜角框）｜ ⬜ 终态。
...
| 架构冻结 | `architecture_freeze` | `final_architecture` | 冻结通过评估的方案，输出追踪矩阵与 ADR（架构决策记录）。运行时产物，盖血缘标识。 |
...
- 约束放宽（Constraint Relaxation）必须显式标记为需要审批，节点不能自行批准。
...
由架构命令统一驱动（不使用裸 `agent-workflow run`），按 `collect → [人工门：血缘] → workflow → attach` 运行：
