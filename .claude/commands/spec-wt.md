...
| worktree 目录 | `F:\lm-wt\<module>`（与主仓同卷，便于硬链接/合并） |
| 分支 | `-b` 指定值，或默认 `feat/<module>` |
| 基线分支 | 当前主仓所在分支（用 `git -C <主仓> rev-parse --abbrev-ref HEAD` 获取） |
...
6. 计算 `<wt_path> = F:\lm-wt\<module>`。

### 2. 预检查

```powershell
# 主仓必须干净或至少基线分支可用；检查 worktree 目录与分支是否已存在
git -C F:\listing-management worktree list
git -C F:\listing-management branch --list '<branch>'
```
...
```powershell
git -C F:\listing-management worktree add 'F:\lm-wt\<module>' -b '<branch>'
```
...
$envFile = 'F:\listing-management\.env'
...
> 注：本步是否需要只取决于 `agents.yaml` 的 `command` 写法，与是否用 `agents.tmp.yaml` 无关。当前主仓 `agents.yaml` 已是直接命令（`cc-opus`/`cc-deepseek`/`codex`，保留个人 wrapper），**默认无需注入，本步跳过**。另两条免注入路径：① 启动时加 `--agents F:\listing-management\workflows\spec-dev\agents.tmp.yaml`（命令写死为 `claude`/`codex`，开箱即用但丢失个人 wrapper）；② 把 `agents.yaml` 改回 `{OPUS_COMMAND}` 占位符版时，才需回到上面的注入步骤。
...
$collectOutput = & C:/Users/12108/miniconda3/python.exe $collectArgs
```
...
下一步（请在确认无误后手动执行）：
  ...
  git -C F:\lm-wt\<module> status            # 提交前核对改动范围
  git -C F:\lm-wt\<module> commit -m '<模块>: <简述>（run <run_id>）'

  # 2. 回主仓合并、清理
  cd F:\listing-management
  git merge <branch>          # 或先开 PR 评审
  git worktree remove F:\lm-wt\<module>   # 合并后清理
  git branch -d <branch>
```
...
=== spec-wt 失败 ===
run_id: <run_id>
worktree 已保留: F:\lm-wt\<module>（请勿删除，重试需要）

重试（从中断节点续跑）：
  C:/Users/12108/miniconda3/python.exe -m agent_workflow.cli retry -r <run_id> --dispatch `
    -w 'F:\listing-management\workflows\spec-dev\workflow.yaml'
```
...
> - **`--run-root` 在主仓目录里敲可省**：找 run 靠当前目录下的 `docs/runs` / `run_index.json`，主仓能直接定位。若在主仓之外的目录敲，才需补 `--run-root 'F:\listing-management\docs\runs'`。
...
1. 使用 **PowerShell** 工具执行所有命令；Python 固定 `C:/Users/12108/miniconda3/python.exe`。
2. 所有 `git` 命令对主仓用 `git -C F:\listing-management`，避免依赖当前目录。
...
| 目录已手删但登记残留 | `worktree list` 仍显示该项 | `git -C F:\listing-management worktree prune` 清理登记 |

### agent 或人工接手完成工作怎么办

适用场景：工作流失败/卡死/产物不达标，由 agent 或人工接手把模块做完。**关键约束（因 spec-dev 不自动 commit）**：

1. **改动只留在 worktree 的 feat 分支**——不要跑到主仓 `F:\listing-management` 直接改。否则破坏隔离，且该 worktree 一旦被 `retry` 续跑，会与主仓分叉、互相覆盖。
2. **接手完成后在 worktree 手动 commit**（工作流不会替你提交）：
   ```powershell
   git -C F:\lm-wt\<module> add -A
   git -C F:\lm-wt\<module> commit -m '<模块>: 人工/agent 接手完成（run <run_id>）'
   ```
...
2. **查 `docs/worktree_map.json`**（第 3a 步写入的映射）：
   ```powershell
   Get-Content 'F:\listing-management\docs\worktree_map.json' | ConvertFrom-Json
   ```
...
4. **若 worktree 目录已被删除但分支还在**：从映射文件拿到 branch 名，在主仓 `git -C F:\listing-management branch --list '<branch>'` 确认，然后按需重建 worktree 或直接在主仓操作该分支。
...
```mermaid
flowchart LR
    MAIN["主仓<br/>F:\listing-management<br/>（配置源 + 产物收口）"]

    subgraph WTS[" "]
        direction TB
        WTA["worktree A<br/>F:\lm-wt\&lt;moduleA&gt;<br/>（工作流运行 · 代码隔离）"]
        WTB["worktree B<br/>F:\lm-wt\&lt;moduleB&gt;<br/>（工作流运行 · 代码隔离）"]
    end
...
| 运行时行为 | 落在哪个目录 | 由什么决定 |
|-----------|-------------|-----------|
| 引擎读 workflow.yaml / agents.yaml / skills 配置 | **主仓** `F:\listing-management` | `-w` + 「配置只放主仓」约定 |
| agent 进程工作目录（cwd）、`project_root` | **worktree** `F:\lm-wt\<module>` | `-p`（run 启动时定死，中途不可切） |
